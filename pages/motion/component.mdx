...
**SVG note:** For SVG components, `x` and `y` are positional attributes distinct from `translateX` and `translateY`. Using `attrX`/`attrY` for the former will change the element's `x`/`y` attributes, while using the latter will set its `style.transform` property.
